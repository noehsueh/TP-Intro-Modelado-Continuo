...
begin 
	using Plots, DifferentialEquations, Random, Distributions
end
...
Genera los datos iniciales (i.e.: posición y velocidad en 2D)
- `n` es la cantidad de partículas y 
- `v` parámetro de escala. 
La función devuelve una matrix ``U`` de tamaño ``4\\times n`` donde la fila 1 corresponde a la posición en ``x``, la fila 2 a la posición en ``y``, la fila 3 a la velocidad en ``x`` y la fila 4 a la velocidad en ``y``. 

"""
function generar_datos(n, v)
	
	function a_cartesianas_x(α, v)
		x = v * cos(α)
	end
	function a_cartesianas_y(α, v)
		y = v * sin(α)
	end

	U = []
	
	A   = rand(Float64, (n,1))*2*pi
	V   = rand(Float64, (n,1))*v
	X₀  = rand(Float64, (n,1))*L
	Y₀  = rand(Float64, (n,1))*L
	V_x = a_cartesianas_x.(A, V)
	V_y = a_cartesianas_y.(A, V)
	U   = transpose(hcat(X₀, Y₀, V_x, V_y))
...
# ╔═╡ da1e7451-bfdb-47aa-8db2-46d72fa85b1b
function dist_cuad(x, y)
		(x[1]-y[1])^2 + (x[2] - y[2])^2
end 
...
Crea el sistema de ecuaciones diferenciales para ser resuelto por el solver. El sistema queda como matrix de ``4 \\times n`` donde cada columna corresponde al sistema de ecuaciones para la partícula i-ésima, cada sistema está dada como:
```math
\\begin{align}
\\dot{x} &= v_x \\\\  
\\dot{y} &= v_y \\\\
\\ddot{x} &= \\frac{1}{m_i} \\boldsymbol{F}_x \\\\
\\ddot{y} &= \\frac{1}{m_i} \\boldsymbol{F}_y \\\\
\\end{align} \\qquad \\boldsymbol{F}= \\sum_{j \\neq i} F_i^j
```
...
"""
 function sistema(du, U, p, t)
	...
	function fuerza_total(i, posi_x, posi_y)
	# Funcion que calcula la suma total de las fuerzas para partícula i 
		fuerza_x = 0
		fuerza_y = 0 
		for k in 1:n
			if k != i 
				posk_x, posk_y, v_x, v_y= U[:, k]
				dist6 = dist_cuad([posi_x, posi_y],[posk_x, posk_y])^3
				fuerza_x += 4*k₁*m[k]*(posi_x - posk_x)/dist6
				fuerza_y += 4*k₁*m[k]*(posi_y - posk_y)/dist6
			end
		end
		fuerza_x, fuerza_y
	end
	...
	for i in 1:n
		du_i = view(du, :, i)
		p_x, p_y, v_x, v_y= U[:, i] # datos de la partícula i 
		fuerza_x, fuerza_y = fuerza_total(i, p_x, p_y)
		du_i[1] = v_x # vel en x de la part i
		du_i[2] = v_y # vel en y de part i
		du_i[3] = fuerza_x  # aceleración en x 
		du_i[4] = fuerza_y # acelaración en y
	end
 end
...
# condición
function condicion_fuera(U, t, integrator)
	POS_X = U[1,:] 
	POS_Y = U[2,:]
	n = size(U, 2)
	esta_fuera = []
...
# rebotes
function  rebotar!(integrator)
	n = size(integrator.u, 2)
	for i in 1:n
		p_x = integrator.u[1,i] 
		p_y = integrator.u[2,i]
		vx = integrator.u[3,i]
		vy = integrator.u[4,i]
		if p_x <= 0
			integrator.u[1,i]  = -p_x
			integrator.u[3,i] = - vx
		end
		if p_y <= 0
			integrator.u[2,i] = -p_y
			integrator.u[4,i] = -vy
		end
		if p_x >= L
			integrator.u[1,i]  = L-(p_x - L)
			integrator.u[3,i] = -vx
		end
		if p_y >= L
			integrator.u[2,i] = L-(p_y - L)
			integrator.u[4,i] = -vy
		end
	end
end
...
# simulación
function simulacion(n, v, k₁, m, t; abs_tol=nothing, rel_tol=nothing)
	param  = [k₁, m]
	u₀ = generar_datos(n, v)
	p  = ODEProblem(sistema, u₀, t, param)
	cb = DiscreteCallback(condicion_fuera, rebotar!)
	if isnothing(abs_tol) && isnothing(rel_tol)
		s  = solve(p, callback=cb, dtmax=0.2)
	else
		s  = solve(p, callback=cb, abstol=abs_tol, reltol=rel_tol, dtmax=0.2)
	end
	return s
end

# ╔═╡ b270f32f-c9b0-49a6-aac5-dc48c1fdb3c6
function simular_dos()
	s = simulacion(2, 1, 1e-3,ones(2), (0,100))
	@gif for i in 0:0.3:100
		u = s(i)
		pos_x = u[1,:]
		pos_y = u[2,:]
		scatter(pos_x, pos_y, 
...
# ╔═╡ 75787583-0e80-42d5-a635-825fc4c74413
md"""
Simulación con dos partículas, observamos que las partículas no salen de la caja y además se replan cuando está cerca (la repulsión se observa para casos de ``k\\_1`` grandes).
"""

# ╔═╡ 304618b6-78af-4126-890a-6eb73dd8599c
simular_dos()
...
# animación
function generar_animacion(s, m, t; step=0.3)
	n = length(m)
	@gif for i in t[1]:step:t[2]
		u = s(i)
		pos_x = u[1,:]
		pos_y = u[2,:]
		n = size(pos_x,1)
...
# ╔═╡ 7cbd4d7f-c15d-4b77-9a3d-9b6689168ce1
const k = 1e-6

# ╔═╡ 71f50951-09c6-4d98-85e6-55f91dff3480
begin # prueba con 2
	n2 = 2
	m2 = ones(2)
	tspan_2 = (0, 100)
	s2 = simulacion(n2, 1, k, m2, tspan_2)
	generar_animacion(s2, m2, tspan_2)
end
...
begin 
	...
	m50 = ones(50)
	tspan_50 = (0, 100)
	s50 = simulacion(n50, 1, k, m50, tspan_50)
	generar_animacion(s50, m50, tspan_50)
end

# ╔═╡ 29da6749-52fb-4881-88d8-556613629dac
# prueba con 50 o 100 de masas distintas
begin # 100 con masas distintas
	n100 = 100
	m100 = rand(Uniform(0.5, 2.5), 100)
	t100 = (0,100)
	s100 = simulacion(n100, 1, k, m100, t100)
	generar_animacion(s100, m100, t100)
end
...
begin
# energia potencial en un instante
	function energia_potencial(u, m, k₁)
		n = size(u, 2)
		pos_x = u[1,:]
		pos_y = u[2,:]
		energia = 0
		for i in 1:n
			for j in 1:n
				if j != i
					dist2 = dist_cuad([pos_x[i], pos_y[i]], [pos_x[j], pos_y[j]])
					energia += k₁ * (m[i]*m[j])/(dist2^2)
				end
			end
		end
		energia
...
# energia cinetica
	function energia_cinetica(u, m, k₁)
		n = size(u, 2)
		v_x = u[3,:]
		v_y = u[4,:]
		energia = 0
		for i in 1:n
			energia += m[i] * (v_x[i]^2 + v_y[i]^2)
		end
		return (1/2) * energia
	end

# energia total, devuelve tupla de energia potencial y cinética
	function energia(u, m, k₁)
		EP = energia_potencial(u,m,k₁)
		EC = energia_cinetica(u,m,k₁)
		return EP, EC
	end

end

# ╔═╡ 47bca8a0-d550-4b3a-a85c-f25310ded299
md"""
Realizamos una simulación de 5 partículas con masas iguales, bajamos el `abstol` y `reltol` para que disminuir el error númerico, de esta forma haremos que la energía mecánica se conserve relativamente. Sin este cambio se notarían fluctuaciones en la energía mecánica.
"""

# ╔═╡ 813ed18f-6a2e-4d9d-8f4b-0d649098627d
begin
	n5 = 5
	m5 = ones(5)
	t5 = (0,200)
	s5 = simulacion(n5, 1, k, m5, t5; abs_tol=1e-10, rel_tol=1e-10)
	ep0, ec0 = energia(s5(0), m5, k)
	e0 = ep0 + ec0
	generar_animacion(s5, m5, t5)
end

# ╔═╡ bd291714-bcc3-445f-bf15-a48a89fef5b7
begin 
	EP5 = []
	EC5 = []
	E5  = []
	...
	for t in tfino
		u = s5(t)
		ep, ec = energia(u, m5, k)
		push!(EP5, ep)
		push!(EC5, ec)
		push!(E5, ep+ec)
	end
	fig1 = plot(tfino, EP5, title="Energía potencial", label=false)
	fig2 = plot(tfino, EC5, title="Energía cinética",  label=false)
	fig3 = plot(tfino, E5,  title= "Energía mecánica",  label=false)
	plot(fig1, fig2, fig3, layout=(3,1))
end

# ╔═╡ e67f912b-a202-426c-9c77-bd0acfd7fe90
md"""
Notamos que la energía mecánica es casi constante, se mantiene cerca del valor inicial $(round(E5[1];digits=2)) con un error relativo de $(round((maximum(E5)-E5[1])/E5[1]*100;digits = 2))% en el pico máximo, además hay pequeñas fluctuaciones debido al error numérico. Con respecto al pico de energía, esto ocurre cuando hay dos particulas que se encuentran muy cercas, haciendo que la energía potencial sea alta (cf. figura de abajo con ``t=`` $(tfino[findmax(EP5)[2]]) ). 
"""

# ╔═╡ 123a8930-5a39-443c-9c33-788d4f02d02d
generar_animacion(s5, m5, [tfino[findmax(EP5)[2]], tfino[findmax(EP5)[2]]])
